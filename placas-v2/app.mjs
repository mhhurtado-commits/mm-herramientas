...
function renderDataControls() { const variant = activeVariant(); const visible = ['dato-clave', 'claves'].includes(state.selectedTemplate); $('#dataControls').classList.toggle('is-hidden', !visible); if (!visible || !variant) return; const facts = variant.datos_clave || (variant.contexto ? [{ label: '', value: variant.contexto, detail: '' }] : []); $('#dataFactList').innerHTML = [0, 1, 2].map(index => { const fact = facts[index] || { label: '', value: '', detail: '' }; return `<div class="field-row data-fact-row"><input type="text" data-fact-index="${index}" data-fact-key="label" value="${fact.label || ''}" placeholder="Etiqueta" aria-label="Etiqueta del dato ${index + 1}"><input type="text" data-fact-index="${index}" data-fact-key="value" value="${fact.value || ''}" placeholder="Dato ${index + 1}" aria-label="Valor del dato ${index + 1}"><input type="text" data-fact-index="${index}" data-fact-key="detail" value="${fact.detail || ''}" placeholder="Detalle opcional" aria-label="Detalle del dato ${index + 1}"></div>`; }).join(''); document.querySelectorAll('[data-fact-index]').forEach(input => input.addEventListener('input', event => { const index = Number(event.target.dataset.factIndex); const facts = variant.datos_clave || []; facts[index] = facts[index] || { label: '', value: '', detail: '' }; facts[index][event.target.dataset.factKey] = event.target.value; variant.datos_clave = facts; render(); })); }
...
function render() {
  ...
  $('#sourceLabel').textContent = variant.fuente?.url ? `Fuente · ${variant.fuente.url}` : 'mediamendoza · Placas V2';
...
$('#socialQuestionInput').addEventListener('input', event => setActiveText('pregunta_social', event.target.value || '¿Qué opinás?'));
