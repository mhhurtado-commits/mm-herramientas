...
function reelCargarLogoPredefinido(nombreArchivo) {
  const img = new Image();
  img.onload = () => {
    RE.logoImg = img;
    reelResetEl('logo');
    localStorage.setItem('mm_logo_predefinido_social', nombreArchivo);
    reelRender();
  };
  img.onerror = () => {
    ...
    fallbackImg.onload = () => {
      RE.logoImg = fallbackImg;
      reelRender();
    };
    fallbackImg.src = '../assets/logo.png';
  };
  img.src = '../assets/' + nombreArchivo;
}

function reelCambiarLogoPredefinido(valor) {
  reelCargarLogoPredefinido(valor);
}

function reelCargarLogoInicial() {
  const guardado = localStorage.getItem('mm_logo_predefinido_social') || 'logo.png';
  reelCargarLogoPredefinido(guardado);
  const select = document.getElementById('reSelectPredefinedLogo');
