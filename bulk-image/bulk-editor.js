...
// ============================================================
// FUNCIONES PARA EL LOGO PREDEFINIDO
// ============================================================

function cargarLogoPredefinidoBulk(nombreArchivo) {
  const img = new Image();
  img.onload = () => {
    logoImg = img;
    localStorage.setItem('mm_logo_predefinido_bulk', nombreArchivo);
    if (logoElement.visible) updatePreview();
    const select = document.getElementById('bulkSelectPredefinedLogo');
    if (select) select.value = nombreArchivo;
  };
  img.onerror = () => {
...
function cambiarLogoPredefinidoBulk(valor) {
  cargarLogoPredefinidoBulk(valor);
}

// Hacer la función disponible globalmente para el HTML
window.cambiarLogoPredefinidoBulk = cambiarLogoPredefinidoBulk;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Inicializar logo predefinido guardado
  const logoGuardadoBulk = localStorage.getItem('mm_logo_predefinido_bulk') || 'logo.png';
  cargarLogoPredefinidoBulk(logoGuardadoBulk);
