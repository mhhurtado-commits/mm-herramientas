...
function reelEditorInit() {
  ...
  function cargarLogoResumen(nombreArchivo) {
  const img = new Image();
  img.onload = () => {
    RE.logoImg = img;
    reelResetEl('logo');
    localStorage.setItem('mm_logo_predefinido_resumen', nombreArchivo);
    reelRender();
    // Sincronizar el select si existe
    const select = document.getElementById('resSelectPredefinedLogo');
  ...
  img.onerror = () => {
    ...
    fallback.onload = () => { RE.logoImg = fallback; reelRender(); };
    fallback.src = '../assets/logo.png';
  };
  img.src = '../assets/' + nombreArchivo;
}

function cambiarLogoResumen(valor) {
  cargarLogoResumen(valor);
}

// Cargar el logo guardado o el default
const logoGuardado = localStorage.getItem('mm_logo_predefinido_resumen') || 'logo.png';
cargarLogoResumen(logoGuardado);
